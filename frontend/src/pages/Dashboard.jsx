...
export default function Dashboard() {
  ...
  useEffect(() => {
    const resultFromState = location.state?.assessmentResult;
    
    if (resultFromState) {
      console.log('📊 Using data from navigation state:', resultFromState);
      setData(resultFromState);
      setIsLoading(false);
      return;
    }

    // If no data in state, check localStorage or fetch latest
  ...
  const profilePreview = data?.profile_preview || data || {};
  const domainScores = profilePreview.domain_scores || {};
  const age = profilePreview.age || 'N/A';
  const sex = profilePreview.sex || 'Not specified';
  const idNo = profilePreview.id_no || 'MS-ANONYMOUS';
  const schemaVersion = profilePreview.schema_version || '3.8';
  ...
  const selfEsteemPercentage = selfEsteemData.score && selfEsteemData.max_possible_score 
    ? (selfEsteemData.score / selfEsteemData.max_possible_score) * 100 
    : 0;
  ...
  // Domain status cards data (moved to useMemo)
              ...
              <div className="space-y-6">
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="font-semibold text-primary">{digitalData.predicted_total_internet_addiction || 0}/40</span>
                  </div>
                  <div className="w-full bg-secondary rounded-full h-2.5 overflow-hidden shadow-inner">
                    <div 
                      className="bg-primary h-full rounded-full transition-all duration-1000"
                      style={{ width: `${((digitalData.predicted_total_internet_addiction || 0) / 40) * 100}%` }}
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="w-full bg-secondary rounded-full h-2.5 overflow-hidden shadow-inner">
                    <div 
                      ...
                      style={{ width: `${digitalData.loneliness_score || 0}%` }}
                    />
                  </div>
                </div>
                <div className="pt-2">
            ...
            <CardContent>
              ...
              <div className="w-full bg-secondary rounded-full h-2.5 overflow-hidden shadow-inner">
                <div 
                  ...
                  style={{ width: `${((burnoutData.burnout_index || 0) / 10) * 100}%` }}
                />
              </div>
            </CardContent>
          </Card>
