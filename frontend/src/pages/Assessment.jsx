...
export default function Assessment() {
  ...
  const sectionAnswered = currentSection
    ? currentSection.questions.filter(q => {
        // Check if question is answered based on input type
        if (q.inputType === "time") {
  ...
  const handleSubmit = async () => {
    ...
    const allData = {
      age: parseInt(age),
      gender: gender,
      ...responses // Spread all responses
    };
    
    console.log("All Data:", allData);
    
    // Simulate API delay and navigate to results/dashboard
    setTimeout(() => {
  ...
  const renderQuestion = (q, index) => {
    ...
    // Scale input (1-10 for occupational burnout)
    if (q.inputType === "scale") {
      return (
        ...
        >
          ...
          <div className="flex gap-4 text-xs font-medium text-muted-foreground mt-3 uppercase tracking-wider">
            <span className="bg-secondary/50 px-2 py-1 rounded-md">Low</span>
            <span className="bg-secondary/50 px-2 py-1 rounded-md">Extreme</span>
          </div>
        </motion.div>
      );
    }

    // Regular scale-based question
    return (
      <QuestionCard
        key={q.id}
        question={q}
  ...
  const bgColors = [
    "rgba(0,0,0,0)", // Step 0: Intro
    "rgba(99, 102, 241, 0.15)", // Step 1: Personality (Indigo)
    "rgba(14, 165, 233, 0.15)", // Step 2: Self-Esteem (Sky)
    "rgba(16, 185, 129, 0.15)", // Step 3: Mood & Sleep (Emerald)
    "rgba(168, 85, 247, 0.15)", // Step 4: Digital & Social (Purple)
    "rgba(245, 158, 11, 0.15)", // Step 5: Burnout (Amber)
    "rgba(244, 63, 94, 0.15)",  // Step 6: Clinical (Rose)
  ];

  return (
    <div className="flex min-h-screen flex-col relative overflow-hidden">
      {/* Soft background gradient shift */}
      <motion.div
...
// export default function Assessment() {
//   const navigate = useNavigate();
//   const [step, setStep] = useState(0); // 1 = intro, 2-6 = sections, 6 = submitting
//   const [age, setAge] = useState("");
//   const [responses, setResponses] = useState({});
//   const [isSubmitting, setIsSubmitting] = useState(false);
...
//   const canProceed = useMemo(() => {
//     if (step === 0) return age && parseInt(age) >= 13 && parseInt(age) <= 100;
//     if (currentSection) return sectionAnswered === currentSection.questions.length;
//     return false;
//   }, [step, age, sectionAnswered, currentSection]);
...
//     //   scores,
//     //   predictions: {
//     //     depression: scores.phq9 >= 10 ? 1 : 0,
//     //     anxiety: scores.gad7 >= 10 ? 1 : 0,
//     //     stress: scores.pss >= 14 ? 1 : 0,
//     //     low_wellbeing: scores.who5 <= 13 ? 1 : 0,
//     //     internet_addict: scores.iat >= 20 ? 1 : 0,
//     //   },
//     //   status: "completed",
//     // });
...
//                     <Brain className="w-8 h-8 text-primary" />
//                   </div>
//                   <CardTitle className="text-2xl font-display">Mental Health Assessment</CardTitle>
...
//                         <div className="text-xs text-muted-foreground">{s.questions.length} questions</div>
//                         <div className="text-sm font-medium mt-1">{s.title.split("(")[0].trim()}</div>
//                       </div>
//                     ))}
//                   </div>
